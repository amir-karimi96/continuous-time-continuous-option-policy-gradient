...
num_ep = 500
continuous_env = D2C(discrete_env= env, low_level_funciton = low_level_function, rho = agent.rho, precise=True, eval_mode=True)
...
j = 0
print(new_movements, durations, len(frames))
for i,f in enumerate(frames):
    ...
    x = x+durations[i]
    print(f.shape)
    frames2.append(get_concat_v(Image.fromarray((f * 255).astype(np.uint8)), im_d))
    print(i)

imageio.mimwrite(os.path.join('./videos/', 'random_agent.mp4'), frames2, fps=10)

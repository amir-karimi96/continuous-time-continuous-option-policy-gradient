...
exps_dir = '/home/franka/project/CTCO_scratch/CTCO_Experiments'
exp_name1 = 'SAC_franka_sparse'
exp_name2 = 'franka_CTCO_sparse'
...
for i, (R_mean, R_std) in enumerate(zip(Returns_2, Returns_errors_2)):
    label = 'CTCO'#.format(configs_2[i]['param']['D_max'])
    if i in [1]:
        p = ax[0].plot(R_mean, label = label)
        ax[0].fill_between(range(R_mean.shape[0]), R_mean-R_std, R_mean+R_std, alpha = 0.3, color = p[0].get_color())
...
plt.title('Performance of algorithms on visual_reacher')
plt.savefig('franka_task.pdf')
plt.savefig('franka_task.png')
import tikzplotlib
tikzplotlib.save("real_world_result.tex")